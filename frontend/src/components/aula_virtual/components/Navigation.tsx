...
const Navigation: React.FC<NavigationProps> = ({ currentView, setView }) => {
  ...
  const NavLink = ({ view, icon, label, fillIcon = false }: { view: ViewState, icon: string, label: string, fillIcon?: boolean }) => (
    <button
      ...
      className={`flex w-full items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${isActive(view)
        ? 'bg-primary/10 text-primary'
        : 'hover:bg-slate-100 text-slate-600'
        }`}
      title={isCollapsed ? label : ''}
    >
      <span className={`material-symbols-outlined ${fillIcon ? 'fill-1' : ''}`}>{icon}</span>
      {!isCollapsed && <span className={`text-sm ${isActive(view) ? 'font-semibold' : 'font-medium'}`}>{label}</span>}
    </button>
  );
    ...
    <aside className={`flex-shrink-0 flex flex-col bg-white border-r border-slate-200 transition-all duration-300 z-50 h-full ${isCollapsed ? 'w-16' : 'w-64'}`}>
      ...
      <div className="flex flex-col gap-6 p-4 h-full overflow-y-auto">
        ...
        <nav className="flex flex-col gap-2">
          <NavLink view={ViewState.STUDENT_DASHBOARD} icon="calendar_month" label="Mi Agenda" fillIcon />
